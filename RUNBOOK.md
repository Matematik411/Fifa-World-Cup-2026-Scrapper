...
|---|---|---|
| `lineups.json` | **every run during tournament** | `{"teams": {"<nation>": {"confirmed": true/false, "xi": ["name",...], "out": ["name",...]}}}`. **Confirmed XIs come ~1h before kickoff** — an evening run catches them and lets you fix captaincy. `confirmed:true` → players in `xi` are nailed (0.98), others benched (0.10); `confirmed:false` → `xi` treated as predicted (0.90) **and, when the predicted `xi` is full (≥10 names), players NOT in it are demoted to 0.40** — match-specific rotation research beats season-long `nailed` flags (added 2026-07-17 after the bronze-final rotation run); `out` → 0.02. Overrides everything for minutes. Name matching needs first names to agree when both sides carry one ("Theo Hernandez" no longer hits Lucas Hernández); bare surnames still match. |
