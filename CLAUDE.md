...
- **GoPicks** (gopicks.app; rules user-quoted 2026-06-12): correct result 3 + exact home goals 1 + exact away goals 1 — the components **stack** (exact score = 5) and are independent (a wrong result can still score 1–2 via goal counts); 90-min result only; **no KO doubling**; leaderboard tiebreaker = total exact goal picks (the optimizer tie-breaks on it too). User joined 2026-06-12 → matches 1–2 missed (0 pts, in `state.json`). Its EV pick can differ from Nostradamus' — reports show both side by side; brief the user on both lists every run.
